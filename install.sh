sudo apt-get update
sudo apt-get install -y ruby-full rubygems imagemagick 
...
cp config/example.database.yml config/databse.yml
...
# BAM: localhost:3000/
# might need to hit it a few times initially things to cache
